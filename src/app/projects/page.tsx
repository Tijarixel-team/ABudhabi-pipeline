    ...
    <>
      ...
      <section className="py-24 lg:py-32">
        <div className="section-shell grid gap-14 lg:grid-cols-[1.08fr_0.92fr] lg:items-center lg:gap-20">
          <div className="relative min-h-[480px] overflow-hidden rounded-[var(--radius)] lg:min-h-[640px]"><Image src={assets.fieldInstallation} alt="Composite pipeline installed in an industrial facility" fill sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover" /></div>
          <div>
            <SectionHeader title="Experience across demanding applications." text="Application requirements vary by environment. Speak with our team about relevant product systems, service conditions and supporting technical information." />
            <div className="mt-10 border-t border-secondary/20">
              {applicationExperience.map((item) => <div key={item.title} className="border-b border-secondary/20 py-6"><h2 className="text-2xl font-normal text-primary">{item.title}</h2><p className="mt-2 leading-7 text-muted">{item.text}</p></div>)}
            </div>
            <Link href="/contact" className="mt-8 inline-flex items-center gap-2 text-sm font-bold text-secondary transition hover:gap-3 hover:text-primary">Discuss your application <ArrowRight aria-hidden size={17} /></Link>
          </div>
        </div>
      </section>
    </>
